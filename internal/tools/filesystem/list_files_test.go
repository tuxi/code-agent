...
func TestListFilesEmitsDirectoryListingAssets(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "cat_sunset.png"), []byte{0x89, 'P', 'N', 'G'}, 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if len(res.Assets) != 5 {
		t.Fatalf("assets = %d, want 5: %+v", len(res.Assets), res.Assets)
	}
	...
	if byPath["cat_sunset.png"] != "image" {
		t.Fatalf("image asset kind = %q", byPath["cat_sunset.png"])
	}
	...
	if out.Kind != "directory_listing" || out.Path != "." || len(out.Items) != 5 {
