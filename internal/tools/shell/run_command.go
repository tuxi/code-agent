...
func shellOperatorHint(command string) string {
	...
	case strings.Contains(command, "\n"):
		return "no newlines in commands. Use && to chain — e.g. `go build && go test`."
	default:
...
func (t *RunCommandTool) Execute(ctx context.Context, ec tools.ExecutionContext, input json.RawMessage) (tools.ToolResult, error) {
	...
	// Convert backtick command substitution `cmd` to $(cmd) so it flows
	// through the existing $() extraction and classification pipeline.
	command = sandbox.ConvertBackticks(command)
	...
	if sandbox.ContainsChainOperators(command) || sandbox.ContainsCommandSubstitution(command) || sandbox.ContainsAssignment(command) || sandbox.ContainsRedirect(command) {
