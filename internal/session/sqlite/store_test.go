...
func TestStoreSaveIsSnapshot(t *testing.T) {
	store := newStore(t)
	ctx := context.Background()
	sess := sampleSession()
...
func TestStoreStats(t *testing.T) {
	...
	s1 := sampleSession()
	s1.ID = "s1"
	s1.Compactions = []session.CompactionStats{
...
func TestStoreListIncludesCompactionAggregates(t *testing.T) {
	store := newStore(t)
	ctx := context.Background()
	s := sampleSession()
	s.Compactions = []session.CompactionStats{
...
func TestStoreProviderStats(t *testing.T) {
	store := newStore(t)
	ctx := context.Background()
	recs := []session.RequestRecord{
...
func TestStoreRecentRequestsTrace(t *testing.T) {
	store := newStore(t)
	ctx := context.Background()
	if err := store.RecordRequest(ctx, session.RequestRecord{
		Model: "m", Success: true, Attempts: 1, LatencyMs: 5000, At: time.Now().Add(-time.Minute),
		Trace: []session.AttemptRecord{{LatencyMs: 5000, Result: "success"}},
	}); err != nil {
		t.Fatal(err)
	}
	if err := store.RecordRequest(ctx, session.RequestRecord{
		Model: "m", Success: true, Attempts: 2, Retries: 1, TimedOut: true, LatencyMs: 35000, At: time.Now(),
		Trace: []session.AttemptRecord{{LatencyMs: 30000, Result: "timeout"}, {LatencyMs: 5000, Result: "success"}},
	}); err != nil {
		t.Fatal(err)
	}
...
func TestStoreLatencyPercentilesAndHistogram(t *testing.T) {
	store := newStore(t)
	ctx := context.Background()
	for i := 1; i <= 10; i++ {
		if err := store.RecordRequest(ctx, session.RequestRecord{
...
func TestStoreTokenUsageByModel(t *testing.T) {
	store := newStore(t)
	ctx := context.Background()
	recs := []session.RequestRecord{
...
func TestStoreProviderStatsEmpty(t *testing.T) {
