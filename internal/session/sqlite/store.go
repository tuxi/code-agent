...
CREATE TABLE IF NOT EXISTS turn_inputs (
	...
	sender_turn_id TEXT NOT NULL DEFAULT '',
...
func (s *Store) open() error {
	...
	for _, stmt := range []string{
		...
		`ALTER TABLE turn_inputs ADD COLUMN message_id TEXT NOT NULL DEFAULT ''`,
		`ALTER TABLE turn_inputs ADD COLUMN correlation_id TEXT NOT NULL DEFAULT ''`,
		`ALTER TABLE turn_inputs ADD COLUMN intent TEXT NOT NULL DEFAULT ''`,
		`ALTER TABLE turn_inputs ADD COLUMN accepted_seq INTEGER NOT NULL DEFAULT 0`,
...
func (s *Store) saveSessionTx(ctx context.Context, tx *sql.Tx, sess *session.Session) error {
	...
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO sessions (id, model, reasoning_effort, summary, prompt_tokens, context_window, compact_threshold, workspace_path, workspace_root, workspace_rel, workspace_ext_id, name, created_at, updated_at, archived_at, metadata, reference_ledger)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(id) DO UPDATE SET
			model=excluded.model, reasoning_effort=excluded.reasoning_effort, summary=excluded.summary, prompt_tokens=excluded.prompt_tokens,
			...
			reference_ledger=excluded.reference_ledger`,
		sess.ID, sess.Model, sess.ReasoningEffort, sess.Summary, sess.PromptTokens, sess.ContextWindow, sess.CompactThreshold,
		sess.WorkspacePath, sess.Workspace.Root, sess.Workspace.Rel, sess.Workspace.ExtID,
		sess.Name, formatTime(sess.CreatedAt), formatTime(sess.UpdatedAt), formatTime(sess.ArchivedAt), metaJSON, ledgerJSON); err != nil {
	...
	for i, m := range sess.Messages {
		...
		if _, err := tx.ExecContext(ctx, `
			INSERT INTO messages (session_id, seq, role, content, tool_calls, tool_call_id, assets, local_assets, origin_turn_id, web_search_calls, reasoning_content)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			sess.ID, i, string(m.Role), m.Content, toolCalls, m.ToolCallID, assetRefs, localAssetRefs, m.OriginTurnID, webSearchCalls, m.ReasoningContent); err != nil {
...
func (s *Store) Load(ctx context.Context, id string) (*session.Session, error) {
	...
	var createdAt, updatedAt, archivedAt, metaJSON, ledgerJSON, name string
	err := s.db.QueryRowContext(ctx, `
		SELECT id, model, COALESCE(reasoning_effort, ''), summary, prompt_tokens, context_window, compact_threshold, COALESCE(workspace_path, ''),
		       ...
		       COALESCE(name, ''), created_at, updated_at, COALESCE(archived_at, ''), COALESCE(metadata, ''), COALESCE(reference_ledger, '')
		FROM sessions WHERE id=?`, id).
		Scan(&sess.ID, &sess.Model, &sess.ReasoningEffort, &sess.Summary, &sess.PromptTokens, &sess.ContextWindow,
			&sess.CompactThreshold, &sess.WorkspacePath,
			&sess.Workspace.Root, &sess.Workspace.Rel, &sess.Workspace.ExtID,
			&name, &createdAt, &updatedAt, &archivedAt, &metaJSON, &ledgerJSON)
	...
	rows, err := s.db.QueryContext(ctx, `
		SELECT role, content, tool_calls, tool_call_id, COALESCE(assets, ''), COALESCE(local_assets, ''), COALESCE(origin_turn_id, ''), COALESCE(web_search_calls, ''), COALESCE(reasoning_content, '') FROM messages WHERE session_id=? ORDER BY seq`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	for rows.Next() {
		var role, content, toolCalls, toolCallID, assetRefs, localAssetRefs, originTurnID, webSearchCalls, reasoningContent string
		if err := rows.Scan(&role, &content, &toolCalls, &toolCallID, &assetRefs, &localAssetRefs, &originTurnID, &webSearchCalls, &reasoningContent); err != nil {
			return nil, err
		}
		m := model.Message{Role: model.Role(role), Content: content, ToolCallID: toolCallID, OriginTurnID: originTurnID, ReasoningContent: reasoningContent}
		if toolCalls != "" {
