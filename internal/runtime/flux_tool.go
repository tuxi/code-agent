...
func (t *FluxWorkflowTool) InputSchema() json.RawMessage {
	return json.RawMessage(`{"type":"object","properties":{"goal":{"type":"string","description":"A complete description of the known workflow and its success conditions."},"action":{"type":"string","enum":["new","retry"],"description":"new=create and execute; retry=recover a failed workflow from a specific node"},"workflow_id":{"type":"string","description":"Required for retry: the workflow ID to recover"},"task_id":{"type":"integer","description":"Required for retry: the task ID to retry"},"resume_from":{"type":"string","description":"Optional for retry: node name to resume from; empty auto-collects failed roots"},"template":{"type":"string","enum":["cross_workspace_collaboration_v1","cross_workspace_collaboration_v2"],"description":"Deterministic workflow template. v1=parallel dispatch with Map fan-out/fan-in. v2=adds independent reviewer stage per agent with structured VERDICT."},"agents":{"type":"array","minItems":1,"maxItems":8,"description":"Resolved worker assignments.","items":{"type":"object","properties":{"role":{"type":"string"},"session_id":{"type":"string"},"reviewer_session_id":{"type":"string","description":"Required for v2: reviewer session ID"},"workspace_path":{"type":"string"},"message":{"type":"string"},"reviewer_message":{"type":"string","description":"v2: custom reviewer assignment; auto-generated from acceptance if omitted"},"acceptance":{"type":"string","description":"v2: acceptance criteria for the reviewer to check"},"max_iterations":{"type":"integer","minimum":1,"maximum":5,"description":"v2: max implement-review cycles, default 3"},"correlation_id":{"type":"string"},"intent":{"type":"string","enum":["request","notification"]}},"required":["role","session_id","message","correlation_id"],"additionalProperties":false}},"parallelism":{"type":"integer","minimum":1,"maximum":8},"timeout_ms":{"type":"integer","minimum":0,"maximum":3600000}},"required":["goal","template","agents"],"additionalProperties":false}`)
}
...
func (t *FluxWorkflowTool) Execute(ctx context.Context, ec tools.ExecutionContext, input json.RawMessage) (tools.ToolResult, error) {
	...
	def, childDef := crossWorkspaceWorkflowDefinitions(args.Template, workflowID, args.Parallelism)
