...
| `thinking` | v1.0 | 支持持久化的 `thinking` 推理快照事件 |
| `reasoning_streaming` | v1.2 | 支持瞬态 `reasoning_delta` 推理增量事件 |
...
| `reasoning_effort` | string | Thinking budget used for this call: `"low"` / `"medium"` / `"high"` / `"x-high"` / `"max"` / `"off"` (reasoning disabled entirely), or omitted when the model config leaves it unset (provider default). The same value was forwarded to the provider (OpenAI-compatible `reasoning_effort`, Responses `reasoning.effort`, Ollama `think`) |
...
#### `reasoning_delta` (since v1.2)

Streaming provider-visible reasoning chunk. **Not persisted** and has no `seq`.
Append it only to the live reasoning buffer for the matching `invocation_id`.
The subsequent persisted `thinking` event is the authoritative complete snapshot
and MUST replace that buffer rather than be appended to it. If `model_finished`
arrives without a `thinking` snapshot, discard the live buffer (a failed streaming
attempt may have fallen back to a response with no visible reasoning).
...
Complete provider-visible reasoning snapshot for one model invocation. It is
distinct from assistant `content` and may be a provider-generated summary,
omitted, or redacted. **Persisted** with `seq`, recoverable on replay/reconnect.
...
| 3 | 文本与推理事件去重 | `token_delta` / `reasoning_delta`：不持久化、无 seq、重连时不恢复。`turn_finished.text` 是最终正文快照；`thinking.text` 是完整推理快照。客户端收到快照时替换对应 live buffer，不能 append |
