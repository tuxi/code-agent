...
- task/multi-agent 子会话使用通用只读子流：
  `GET /v1/child-streams/{id}/events` + `GET /v1/child-streams/{id}/stream`。
  恢复规则与 job 一致：先以 child 自己的最大 `seq` 补 backlog，再接 WS；父会话仍只接收
  `task_started`/`task_finished` bracket，不接收子 agent 的详细事件。
