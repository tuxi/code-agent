...
| 组件 | 技术选型 | 大小/性能 | 依赖 | 备注 |
|------|---------|----------|------|------|
| 嵌入模型 | Core ML (Mac + iOS 统一) | 130 MB, < 50ms/chunk | Core ML + ANE, Swift 侧推理 | tokenizer 在 Swift 同侧，零 FFI 开销 |
...
```
Mac + iOS 统一: Core ML (Swift 宿主 → gomobile 回调 → Go runtime)

  Mac 和 iOS 采用相同的嵌入式架构（Swift host → gomobile → Go runtime），
  因此 embedding 运行时统一走 Core ML，不需要 ONNX 分支。

  Swift 侧: gte-small → coremltools 转换 → .mlmodelc → Core ML 推理 (ANE)
  Go 侧:   通过 gomobile 回调接口接收 embedding 结果
```
...
```
□ internal/rag/embedder.go       — Embedder 接口 (Go 侧定义，Swift 侧实现)
□ Swift: CoreMLEmbedder.swift    — Core ML 推理 + WordPiece 分词
