...
理由:
  ✅ code-agent 的端侧定位与隐私优先理念一致
  ✅ CLI/TUI: Ollama /api/embed 本地 HTTP, Go 侧自包含
  ✅ iOS/Mac App: Core ML, Swift 侧推理
  ✅ 纯 Go BLOB + 暴力 cosine，无外部服务依赖（决策 5，不用 vec0）
  ⚠️ 云端 embedding API 作为可选项 (Phase 3)
```
...
| 组件 | 技术选型 | 大小/性能 | 依赖 | 备注 |
|------|---------|----------|------|------|
| 嵌入模型 | nomic-embed-text (CLI/TUI) 或 gte-small (Core ML) | 274MB / 130MB | Ollama HTTP 或 Core ML | CLI 路径零额外依赖 |
...
code-agent 有三种部署形态，embedding 来源不同：

```
Mac CLI / TUI (独立 Go binary):
  ./codeagent
  embedding 来源: Ollama /api/embed → 本地 HTTP, Go 侧自包含
  模型: nomic-embed-text (768d, ~274MB) 或 gte-small (384d, ~130MB)

Mac App (Swift host → gomobile):
  AgentKit.app
  embedding 来源: Core ML → Swift 侧推理, gomobile 回调传 Go

iOS App (Swift host → gomobile):
  同上
  embedding 来源: Core ML → Swift 侧推理, gomobile 回调传 Go
```
...
```
         ...
         │           Go: Embedder 接口                   │
         │                                              │
         │  CLI/TUI ──→ OllamaEmbedder (HTTP client)    │
         │  Mac App ──→ CoreMLEmbedder (gomobile cb)    │
         │  iOS App ──→ CoreMLEmbedder (gomobile cb)    │
         │  可选    ──→ CloudEmbedder (HTTP client)      │
         └─────────────────────────────────────────────┘
```

**为什么不用 ONNX**：CLI/TUI 走 Ollama 已经是本地 HTTP、零额外依赖；嵌入式宿主走 Core ML 是 Apple 原生路径。两条路径都不需要把 ONNX runtime 塞进 Go/CGo。ONNX 分支从方案中完全删除。

### 6.2 Embedder 接口

```go
// Go 侧接口定义，Swift 侧通过 gomobile 实现
type Embedder interface {
    Embed(text string) ([]float32, error)
    ModelID() string    // 模型标识，用于版本锁
    Dims() int          // nomic-embed-text: 768, gte-small: 384
    MaxTokens() int     // nomic-embed-text: 8192, gte-small: 512
    Pooling() string    // "mean" / "cls"
}

// Swift 侧实现: CoreMLEmbedder (Mac + iOS 共用)
// Go 侧接收: mobile.Start(..., embedder: Embedder?) 传入
...
gte-small 使用 WordPiece 分词器，Core ML 模型本身不含分词器。分词在 Swift 侧做，与模型推理同侧，零跨 FFI 开销。方案：

1. **Swift 内嵌词表（首选）**：将 `vocab.txt` (~110KB) 打包进 App Bundle，用 Swift 实现 longest-match WordPiece。~200 行代码，精度完全匹配 Python transformers 输出。
2. **NLTokenizer 简化方案**：系统自带 `NLTokenizer` (BPE)，零依赖，精度损失约 5-10%。作为快速验证的兜底。
...
```
□ internal/rag/embedder.go       — Embedder 接口 (Go 侧)
    ├── ollama_embedder.go        — CLI/TUI: Ollama /api/embed HTTP client
    └── (Mac/iOS App: Swift CoreMLEmbedder 通过 gomobile 回调)
□ Swift: CoreMLEmbedder.swift    — App 宿主: Core ML 推理 + WordPiece 分词
