...
func SetTheme(name string) error {
	...
	// Persist the choice to the user-scope settings file so it survives restarts.
	if err := updateConfigTheme(name); err != nil {
		// A failed persist must not fail the live theme switch.
		_ = err
	}
...
// updateConfigTheme persists the theme name to the user-scope settings file,
// preserving every other key (settings.Persist is the canonical writer). A
// missing home directory disables persistence, leaving the choice session-only.
func updateConfigTheme(themeName string) error {
	home, err := os.UserHomeDir()
	if err != nil {
		return err
	}
	path := settings.UserPath(home)
	if path == "" {
		return nil
	}
	return settings.Persist(filepath.Clean(path), func(doc map[string]any) {
		doc["theme"] = themeName
	})
}
