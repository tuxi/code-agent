...
// SessionSelectedMsg is sent when a session is selected. The app resolves the
// ID through its existing /resume flow.
...
type SessionDialog interface {
	tea.Model
	layout.Bindings
	SetSessions(sessions []session.Meta)
		...
		case key.Matches(msg, sessionKeys.Enter):
			if len(s.sessions) > 0 {
				return s, util.CmdHandler(SessionSelectedMsg{
					ID: s.sessions[s.selectedIdx].ID,
				})
			}
...
func (s *sessionDialogCmp) View() string {
	...
	for _, sess := range s.sessions {
		if len(effectiveTitle(sess)) > maxWidth-4 { // Account for padding
			maxWidth = len(effectiveTitle(sess)) + 4
		}
	}
	...
	for i := startIdx; i < endIdx; i++ {
		...
		subtitle := fmt.Sprintf(" · %s · %d msgs · %s", sess.Model, sess.MessageCount, humanAgo(sess.UpdatedAt))
		sessionItems = append(sessionItems, itemStyle.Padding(0, 1).Render(title+subtitle))
	}
...
func (s *sessionDialogCmp) SetSessions(sessions []session.Meta) {
	s.sessions = sessions
...
// Copied verbatim from the root package (sessions.go) so the dialog can render
// session lists without importing the root package.
